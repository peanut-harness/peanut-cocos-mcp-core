...
消化面：`@peanut/cocos-lumen-24`（`Lumen24McpBridge` / `EditorMcpLumen24Bridge`）+ editor-mcp 再导出。
...
-   **已 peel**：Bridge 住在 `@peanut/cocos-lumen-24`；优先名 `Lumen24McpBridge`（兼容导出 `EditorMcpLumen24Bridge`）
...
-   **主路径 reuse** `peanut-asset-catalog` 扫 `assets/**/*.meta`（2.4 meta 含 `uuid`+`importer`，可用）

...
const fixtureRoot = join(packageRoot, 'tests', 'fixtures', 'default_prefab_24');
...
function resolvePrefabSourceRoot(cocosVersion) {
    ...
    if (process.env.ALLOW_MISSING_CREATOR_24_PREFAB === '1' && existsSync(fixtureRoot)) {
        process.stderr.write(
            `bundle_default_prefab_24: Creator ${cocosVersion} not found; using test fixtures ${fixtureRoot}\n`,
        );
        return fixtureRoot;
    }
    throw new Error(
        `bundle_source_missing: install Creator ${cocosVersion} under /Applications/Cocos/Creator/${cocosVersion}/ or set COCOS_CREATOR_24_RESOURCES; CI may set ALLOW_MISSING_CREATOR_24_PREFAB=1`,
    );
}

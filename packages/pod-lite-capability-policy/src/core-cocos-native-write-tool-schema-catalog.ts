...
export class CoreCocosNativeWriteToolSchemaCatalog {
    ...
    private static buildSchemas(): ReadonlyMap<CoreCocosNativeWriteOperation, ICoreMcpJsonSchema> {
        ...
        schemas.set('lumen.structure', Object.freeze({ type: 'object', properties: Object.freeze({ prefabRelativePath: prefabPath(), parentPath: nodePath('父节点路径，必须以 / 开头。'), cocosVersion: this.string('可选 Creator 版本覆盖。'), autoCommit: autoCommit(), approvalId: this.string('本地审批租约 ID。') }), required: Object.freeze(['prefabRelativePath', 'parentPath', 'recipe']), additionalProperties: true }));

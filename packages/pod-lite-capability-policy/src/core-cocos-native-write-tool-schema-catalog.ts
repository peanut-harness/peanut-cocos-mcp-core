...
export class CoreCocosNativeWriteToolSchemaCatalog {
    ...
    private static buildSchemas(): ReadonlyMap<CoreCocosNativeWriteOperation, ICoreMcpJsonSchema> {
        ...
        const control = (): Readonly<Record<string, ICoreMcpJsonSchema>> => ({ approvalId: this.string('本地审批租约 ID（与 approvalToken 等价，指向同一 McpApprovalLeaseStore 租约）。'), approvalToken: this.string('本地审批租约 token（与 approvalId 等价；双传时优先 approvalId）。'), confirmDestructive: this.boolean('破坏性写确认（Hub/Lite 对齐；risk=destructive 时需为 true）。') });
        ...
        schemas.set('lumen.structure', Object.freeze({ type: 'object', properties: Object.freeze({ prefabRelativePath: prefabPath(), parentPath: nodePath('父节点路径，必须以 / 开头。'), cocosVersion: this.string('可选 Creator 版本覆盖。'), autoCommit: autoCommit(), approvalId: this.string('本地审批租约 ID（与 approvalToken 等价，指向同一 McpApprovalLeaseStore 租约）。'), approvalToken: this.string('本地审批租约 token（与 approvalId 等价；双传时优先 approvalId）。') , confirmDestructive: this.boolean('破坏性写确认（Hub/Lite 对齐；risk=destructive 时需为 true）。') }), required: Object.freeze(['prefabRelativePath', 'parentPath', 'recipe']), additionalProperties: true }));

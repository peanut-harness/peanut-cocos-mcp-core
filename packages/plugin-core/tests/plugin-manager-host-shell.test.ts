...
test('plugin-manager host shell should classify panel files as panel remount and core files as kernel reload', async (): Promise<void> => {
    ...
    assert.equal(
        pluginManagerHostShell.resolveChangeStrategy('D:\\workspaces\\peanut-cocos-mcp-projects\\peanut-plugin-panel\\panels\\plugin-manager\\index.html'),
        'panel-ui',
    );
    assert.equal(
        pluginManagerHostShell.resolveChangeStrategy('D:\\workspaces\\peanut-cocos-mcp-projects\\peanut-plugin-core\\src\\app\\plugin-manager-app.ts'),
        'kernel',
    );
    assert.equal(
        pluginManagerHostShell.resolveChangeStrategy('D:\\workspaces\\peanut-cocos-mcp-projects\\peanut-plugin-core\\src\\host\\plugin-manager-host-shell.ts'),
        'ignore',
    );
    ...
    const /* 保存异步操作的解析结果，供当前流程后续校验、转换或编排使用。 */ kernelStrategy = await pluginManagerHostShell.handleChangedPaths([
        'D:\\workspaces\\peanut-cocos-mcp-projects\\peanut-plugin-core\\src\\app\\plugin-manager-app.ts',
    ]);

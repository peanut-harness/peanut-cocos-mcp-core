...
test('plugin-manager host watch path resolver should return panel and source directories from the package root', (): void => {
    // 保存当前执行步骤的中间结果，仅在本作用域内参与后续处理。
    const /* 保存当前执行步骤的中间结果，仅在本作用域内参与后续处理。 */ watchPaths = resolvePluginManagerHostWatchPaths('D:\\workspaces\\peanut-cocos-mcp-projects\\peanut-plugin-core\\');

    assert.deepEqual(watchPaths, [
        'D:/workspaces/peanut-cocos-mcp-projects/peanut-plugin-core/panels/plugin-manager/embedded',
        'D:/workspaces/peanut-cocos-mcp-projects/peanut-plugin-core/src',
    ]);
});
...
test('plugin-manager split host watch path resolver should prefer panel package assets as the source of truth', (): void => {
    ...
    const /* 保存当前执行步骤的中间结果，仅在本作用域内参与后续处理。 */ watchPaths = resolvePluginManagerSplitHostWatchPaths(
        'D:\\workspaces\\peanut-cocos-mcp-projects\\peanut-plugin-core\\',
        'D:\\workspaces\\peanut-cocos-mcp-projects\\peanut-plugin-panel\\',
    );

    assert.deepEqual(watchPaths, [
        'D:/workspaces/peanut-cocos-mcp-projects/peanut-plugin-core/src',
        'D:/workspaces/peanut-cocos-mcp-projects/peanut-plugin-core/panels/plugin-manager/embedded',
        'D:/workspaces/peanut-cocos-mcp-projects/peanut-plugin-panel/panels/plugin-manager/embedded',
        'D:/workspaces/peanut-cocos-mcp-projects/peanut-plugin-panel/src',
    ]);
});

...
export class PluginManagerHostShell {
    ...
    public resolveChangeStrategy(changedPath: string): PluginManagerHostShellChangeStrategy {
        ...
        if (normalizedPath.includes('/peanut-plugin-panel/panels/plugin-manager/')) {
            return 'panel-ui';
        }
        if (normalizedPath.includes('/peanut-plugin-core/src/host/')) {
            return 'ignore';
        }
        if (normalizedPath.includes('/peanut-plugin-core/src/')) {
            return 'kernel';
        }
        return 'ignore';
    }

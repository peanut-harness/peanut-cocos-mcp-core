...
/**
 * @description 解析当前宿主 Creator 版本；未知时回落 2.4.0。
...
async function bootPluginStack(): Promise<void> {
    ...
    hostGlobal.Editor?.log?.(
        `[peanut-pod-24] plugin-stack ready version=${creatorVersion} manager=true`,
    );
...
export function buildDiagnoseSnapshot(): Record<string, unknown> {
    ...
    const activePluginIds =
        pluginManager != null
            ? pluginManager.listRuntimeRecords().map((record) => record.pluginId)
            : [];
    return {

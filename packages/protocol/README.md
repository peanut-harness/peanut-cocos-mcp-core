...
`@peanut/pod-protocol` 是 Peanut 插件运行时 monorepo 的共享契约包，提供跨工程稳定 DTO、字面量类型和纯辅助类型。
...
```typescript
import type { IPluginManifest, ITaskRequest, PluginId } from '@peanut/pod-protocol';
```
...
- 跨 `@peanut/pod-engine/runtime`、`@peanut/pod-engine/installation`、`@peanut/pod-engine/kernel` 与 `@peanut/pod-panel` 共享的 DTO
- 稳定字面量联合类型
...
当前明确不进入 `@peanut/pod-protocol` 的例子：

...
`peanut-runtime` 负责 Creator 版本适配、宿主能力封装以及统一任务执行管线。
...
```typescript
import { RuntimeFacade, VersionResolver } from 'peanut-runtime';
```
...
```bash
npm run --workspace peanut-runtime typecheck
npm run --workspace peanut-runtime test
npm run --workspace peanut-runtime smoke
```

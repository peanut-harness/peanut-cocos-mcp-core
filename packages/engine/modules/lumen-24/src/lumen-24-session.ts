...
/**
 * @description 用 runtime Asset 桥接实现的 2.4 刷新适配器。
 */
export class Lumen24RuntimeAssetRefreshAdapter implements ILumen24EditorRefreshAdapter {
    ...
    /**
     * @description Prefab 写入函数（可选）。
     */
    private readonly _writePrefab: ((relativePath: string, prefab: readonly Record<string, unknown>[]) => Promise<unknown>) | null;

    /**
     * @description 创建适配器。
     * @param refreshOne 单资源刷新
     * @param writePrefab 可选写 Prefab
     */
    public constructor(
    ...
    /**
     * @description 刷新列表；`.prefab` 经 writePrefab 同步 AssetDB（已跟踪 saveExists / 未跟踪 refresh）。
     * @param projectRoot 工程根
     * @param relativePaths 相对路径
     * @returns 结果
     */
        ...
        for (const normalized of refreshTargets) {
            if (this._writePrefab != null && normalized.toLowerCase().endsWith('.prefab')) {
                ...
                if (!existsSync(absolutePath)) {
                    details.push({ path: normalized, skipped: true, reason: 'prefab_missing_on_disk' });
                    continue;
                }
                const parsed = JSON.parse(readFileSync(absolutePath, 'utf8')) as unknown;
                if (!Array.isArray(parsed)) {
                    details.push({ path: normalized, skipped: true, reason: 'prefab_json_not_array' });
                    continue;
                }
                try {
                    const written = await this._writePrefab(normalized, parsed as readonly Record<string, unknown>[]);
                    details.push({ path: normalized, written });
                } catch (error) {
                    details.push({
                        path: normalized,
                        skipped: true,
            ...
            try {
                const refreshed = await this._refreshOne(normalized);
                details.push({ path: normalized, refreshed });
            } catch (error) {
                details.push({
                    path: normalized,
                    skipped: true,
    ...
    /**
     * @description 将 refresh 目标解析为磁盘存在路径；缺失时回落父目录，避免 AssetDB stat ENOENT。
     * @param projectRoot 工程根
     * @param relativePaths 原始相对路径
     * @returns 去重后的可刷新路径
     */

...
import { parseCreatorPidsForProject, resolveDefaultCreatorBinary } from '../../../../scripts/creator-hub-session.mjs';
...
test('creator-hub-session Windows PID parse + default binary helper', async () => {
    const { resolve } = await import('node:path');
    const project = 'D:/workspaces/demo/proj';
    const resolved = resolve(project);
    const text = [
        `111 C:\\ProgramData\\cocos\\editors\\Creator\\3.8.7\\CocosCreator.exe --nologin --project ${resolved}`,
        `222 C:\\ProgramData\\cocos\\editors\\Creator\\3.8.7\\CocosCreator.exe --type=renderer --project ${resolved}`,
    ].join('\n');
    assert.deepEqual(parseCreatorPidsForProject(text, project), [111]);
    const bin = resolveDefaultCreatorBinary();
